bufferedMQF_init(bufferedmqf, nSlots, nSlots, 2*kSize, value_bits,fixedCounterSize,0, "");kDataFrameBMQF::kDataFrameBMQF(uint64_t ksize,uint8_t q,uint8_t fixedCounterSize,uint8_t value_bits,double falsePositiveRate):#include "kDataFrame.hpp"
...
// kDataFrameBMQF_____________________________
...
bool kDataFrameBMQF::insert(string kmer){
  ...
  uint64_t hash=hasher->hash(kmer)% bufferedmqf->memoryBuffer->metadata->range;
  try{
    bufferedMQF_insert(mqf,hash,1,false,false);
  }
  catch(overflow_error & e)
  {
    reserve(bufferedmqf->memoryBuffer->metadata->nslots);
...
bool kDataFrameBMQF::erase(string kmer){
  ...
  bufferedMQF_remove(bufferedmqf->memoryBuffer,hash,currentCount,false,false);
  return true;
}
...
kDataFrameBMQF* kDataFrameBMQF::load(string filePath){
  ifstream file(filePath+".extra");
  uint64_t filekSize;
  file>>filekSize;
 ...
 kDataFrameIterator kDataFrameBMQF::begin(){
     return (kDataFrameIterator(
       (_kDataFrameIterator*)new kDataFrameMQFIterator(bufferedmqf->memoryBuffer,kSize,hasher),
       (kDataFrame*)this));
 }

 kDataFrameIterator kDataFrameBMQF::end(){
   kDataFrameMQFIterator* it=new kDataFrameMQFIterator(bufferedmqf->memoryBuffer,kSize,hasher);
   it->endIterator();
   return (kDataFrameIterator(it,(kDataFrame*)this));
