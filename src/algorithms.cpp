...
//frame = new kDataFrameMAP(kSize);
        ...
        kmerDecoder *KMERS;
        KMERS = new Kmers(seqIn, chunk_size, kSize);
        ...
        while (!KMERS->end()) {
            ...
            for (const auto &read : *KMERS->getKmers()) {
                string readName = read.first;
                ...
                for (const auto &kmer: read.second) {
