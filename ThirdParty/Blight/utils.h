...
kmer_type nuc2int(char c);
kmer_type nuc2intrc(char c);
...
bool kmer_in_superkmer(const kmer_type canon, const vector<kmer_type>& V);
kmer_type min_k(const kmer_type& k1, const kmer_type& k2);
kmer_type str2num(const string& str);
...
class bfile_iterator : public std::iterator<std::forward_iterator_tag, kmer_type> {
  public:
	bfile_iterator()
	  : _is(nullptr)
	  , _pos(0)
	  ...
	  , _cptread(0) {
		_buffsize = 10000;
		_buffer = (kmer_type*)malloc(_buffsize * sizeof(kmer_type));
	}
	...
	bfile_iterator(const bfile_iterator& cr) {
		...
		_is = cr._is;
		_buffer = (kmer_type*)malloc(_buffsize * sizeof(kmer_type));
		memcpy(_buffer, cr._buffer, _buffsize * sizeof(kmer_type));
		_inbuff = cr._inbuff;
	...
	kmer_type const& operator*() { return _elem; }
  ...
  private:
	void advance() {
		...
		if (_cptread >= _inbuff) {
			int res = fread(_buffer, sizeof(kmer_type), _buffsize, _is);
			_inbuff = res;
			_cptread = 0;
	...
	kmer_type* _buffer; // for buffered read
