...
// Use 64 bits int to represent kmer_type up to k=32
#define kmer_type uint64_t



// Use 128 bits int to represent kmer_type up to k=64
// #define kmer_type __uint128_t
...
typedef boomphf::SingleHashFunctor<kmer_type> hasher_t;
typedef boomphf::mphf<kmer_type, hasher_t> MPHF;



struct KmerHasher {
	size_t operator()(const kmer_type& k) const { return ((uint64_t)k); }
};
...
class kmer_Set_Light {
  public:
	...
	Pow2<kmer_type> offsetUpdateAnchor;
	Pow2<kmer_type> offsetUpdateMinimizer;
	...
	void updateK(kmer_type& min, char nuc);
	void updateRCK(kmer_type& min, char nuc);
	void updateM(kmer_type& min, char nuc);
	void updateRCM(kmer_type& min, char nuc);
	void fill_positions(uint64_t beg, uint64_t end, bm::bvector<>& position_super_kmers_local);
	kmer_type update_kmer(uint64_t pos, kmer_type mini, kmer_type input);
	kmer_type get_kmer(uint64_t pos, uint64_t mini);
	void print_kmer(kmer_type num, uint64_t n = 100);
	...
	kmer_type update_kmer_local(uint64_t pos, const vector<bool>& V, kmer_type input);
	string kmer2str(kmer_type num);
	kmer_type regular_minimizer(kmer_type seq);
	void create_super_buckets(const string&);
	void construct_index(const string& input_file, const string& wdir = "");
	vector<kmer_type> kmer_to_superkmer(const kmer_type canon, kmer_type minimizer, int64_t& rank, int64_t& hash);
	int64_t hash_to_rank(const int64_t hash, kmer_type minimizer);
	int64_t kmer_to_hash(const kmer_type canon, kmer_type minimizer);
	...
	kmer_type get_kmer(uint64_t pos);
	void str2bool(const string& str, uint64_t mini);
	void dump_and_destroy(const string& output_file);
	kmer_type regular_minimizer_pos(kmer_type seq, uint64_t& position);
	...
	//~ void get_monocolor_minitigs_mem(vector<robin_hood::unordered_node_map<kmer_type, kmer_context>>& min2kmer2context,zstr::ofstream* out,const vector<int32_t>& mini,uint64_t number_color, int colormode);
...
public:
	...
	string get_kmer_str()const{
		kmer_type seqR = index_ptr->get_kmer(position);
		return index_ptr->kmer2str(seqR);
	}
	
	
	kmer_type get_kmer()const{
