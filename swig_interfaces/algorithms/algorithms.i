...
//    void dumpMQF(QF *memoryMQF, int ksize, std::string outputFilename);
    ...
    void estimateMemRequirement(std::string ntcardFilename, uint64_t numHashBits, uint64_t tagSize, uint64_t *res_noSlots, uint64_t *res_fixedSizeCounter, uint64_t *res_memory);
...
/// Index function without needing the kmerDecoder
    void index(kDataFrame *frame, std::map<std::string, int> parse_params, string filename, int chunks,
